...
const Products = () => {
  ...
  return (
    <div className="ContainerProductsMajor">
      ...
      {showCart && (
        <div className="cart">
          <h2><FontAwesomeIcon icon={faCartShopping} style={{color: "#27902e"}} /></h2>
          <ul>
            {cart.map((item, index) => (
              <li key={index}>
                 <div><img src={`data:image/jpeg;base64,${item.images}`}  alt={item.name} style={{width:'30%'}} /></div>
                <div>{item.name}</div>
                <div>${item.price}</div>
              </li>
            ))}
          ...
          <Button variant="contained" color="secondary" onClick={cancelPurchase}>Cancelar</Button>
          {/* Botón para procesar el pago */}
          <Button variant="contained" color="primary" onClick={processPayment}>Pagar con MercadoPago</Button>
        </div>
      )}
    </div>
  );
